...
function oddPairFirstLast(num) {
    // your code here
    // return (String(num).slice(1, String(num).length-1)) === ''
    if (num === ''){
      return 0
    } else if (num < 10){
      return Number(num + String(1))
    } else {
      if ((String(num)[0] + String(num)[String(num).length -1]) % 2 !== 0){
        return +(String(num)[0] + String(num)[String(num).length -1])  +  oddPairFirstLast(String(num).slice(1, String(num).length-1))
      } else return oddPairFirstLast(String(num).slice(1, String(num).length-1))
    }
}
// console.log(oddPairFirstLast(12)) // 25
