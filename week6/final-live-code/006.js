...
function pairPrime(num) {
  ...
  if (num === 0){
    ...
    // console.log(+(String(num)[0] + String(num)[String(num).length - 1]))
    for (let i = 2 ; i< (String(num)[0] + String(num)[String(num).length - 1]) ; i++){
      if (+(String(num)[0] + String(num)[String(num).length - 1]) % i === 0){
          return 0 + pairPrime(+(String(num).slice(1 , String(num).length - 1)))
      }
    }

    return 1 + pairPrime(+(String(num).slice(1 , String(num).length - 1)))
